...
- Added an Ubuntu 22.04/24.04 Linux release-build helper with explicit Node.js version validation, safe `sh`-to-Bash handoff, working-directory-independent path resolution, and DEB/RPM-only packaging.
- Corrected the branded native PNG dimensions and split Tauri icon manifests by platform, preventing Windows/macOS icon containers from being copied into Linux packages. The supported Linux release command now produces DEB and RPM packages only; AppImage packaging is deferred because the current `linuxdeploy` path remains unreliable.
- Replaced operating-system emoji glyphs in application buttons with compact inline SVG icons so toolbar, welcome, zoom, and Retro sound controls render consistently on minimal Linux installations. Retro startup audio now requests a playback-oriented Web Audio buffer to reduce underruns on virtualized and low-end audio devices.
- Fixed Code and Preview remaining constrained to the previous horizontal split heights after leaving Split mode. Only the horizontal divider ratio is retained now; single-pane modes always reclaim the full content height.
...
- Fixed the page size setting doing nothing. `SettingsController` set a `data-page-size` attribute that no rule ever matched, while the stylesheets read a `--current-page-width` custom property that only `UIController` set, and only at startup. Choosing A4, Letter, or A3 now takes effect immediately, in centered layout and distraction-free alike.
- Centered layout no longer applies in Split mode, where narrowing each of two side-by-side panes to a page width left two thin columns. It applies in Code and Preview as before, and returns as soon as you leave Split.
- Made the status bar tab manager reachable with more than one document open. Its button previously appeared only past nine tabs, although the manager offers search, reordering, and per-tab commands well before that. The dropdown still lists the nine most recent.
- Gave centered layout and page size a single write path in `SettingsController`, removing the second unused copy of both in `UIController`.
- Split the 3,724-line `src/styles.css` into eighteen component-owned parts under `src/styles/base/`, leaving the base stylesheet as an ordered `@import` manifest with no rules of its own. The parts keep their original order, so the cascade is unchanged: the flattened stylesheet has the same 545 rules in the same order, and the production bundle is byte-identical to the one the single file produced.
- Added `scripts/css-order.mjs`, which flattens a stylesheet through its `@import` rules into the order a browser would apply them, so a CSS move can be shown not to change the cascade.
- Moved the horizontal split stylesheet out of `HorizontalSplitPlugin` into `plugins/horizontalSplitStyles.js`, taking the plugin from 911 to 727 lines. The emitted stylesheet is byte-identical, and a test now pins that the plugin mounts exactly it and removes it on destroy.
- Extracted the tab context menu and tab search modal decisions into a pure `components/tabChrome.js` — menu placement clamping, per-tab command availability, search matching, and modal keyboard navigation — and replaced the inline branches in `TabUIController` with a small dispatch.
...
- Rebuilt the Retro theme against the Windows 3.1 palette and three bevel primitives. `outset` and `inset` borders are gone: their light and dark sides are derived by an implementation-defined algorithm and are not guaranteed to match between WebView2 and WebKitGTK. Modal headers are now title bars with a centred caption and a control-menu box, and the theme gained a navy selection, dotted focus rectangles, engraved disabled text, and dithered scrollbar tracks.
- Added a Retro-only **Desktop Colour** setting offering the eight Windows 3.1 desktop colours, applied to the welcome screen and to the area around the document in centered layout and distraction-free mode. Each colour carries the text colour that stays legible on it.
- Fixed inline code rendering at roughly 10px in Retro. Fenced code was given an explicit size earlier; inline code was missed and kept inheriting the shared 85% rule off Retro's compact document font.
