...
- `retroDesktop`: the eight Windows 3.1 desktop colours, each with the text colour that stays legible on it, because the welcome screen draws its heading straight onto this background. `SettingsController` owns the preference and writes `--retro-desktop` and `--retro-desktop-text`; only `styles/themes/retro.css` reads them, so the properties are inert under the other themes and the setting is simply hidden.
